function [minimum_point, time_elapsed] = FeasibleDirectionsMethod(X0, precision, debug_mode)
    ...
    while eps <= x(1)
        ...
        if getActiveConstr(X0) == 1
            A = [1 -constrGrad1(1) -constrGrad1(2); 1 objFunGrad(1) objFunGrad(2)];
        else
            A = [1 -constrGrad2(1) -constrGrad2(2); 1 objFunGrad(1) objFunGrad(2)];
        end
